...
public class Rook extends AbstractPiece implements Movable{
	...
	private void getRankCandidates(List<Location> moveCandidates, Map<Location, Square> squareMap, Location current, int offset) {
		try {
			...
			while(squareMap.containsKey(next)) {
				...
				moveCandidates.add(next);
				next=LocationFactory.build(next, next.getFile().ordinal(), 0);
			}
		}catch(Exception e) {}
	}
	
	private void getFileCandidates(List<Location> moveCandidates, Map<Location, Square> squareMap, Location current, int offset) {
		try {
			...
			while(squareMap.containsKey(next)) {
				...
				moveCandidates.add(next);
				next=LocationFactory.build(next, -1, 0);
			}
		}catch(Exception e) {}
	}
